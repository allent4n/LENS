...
WEIGHTS_NAME = 'audio_pytorch_model.bin'
...
class AudioSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # Apply the attention mask is (precomputed for all layers in AudioModel forward() function)
...
class AudioLMPredictionHead(nn.Module):
    def __init__(self, config, Audio_model_embedding_weights):
        ...
        self.weight = Audio_model_embedding_weights
        self.bias = nn.Parameter(torch.zeros(Audio_model_embedding_weights.size(1)))
...
class AudioOnlyMLMHead(nn.Module):
    def __init__(self, config, Audio_model_embedding_weights):
        ...
        self.predictions = AudioLMPredictionHead(config, Audio_model_embedding_weights)
...
class AudioModel(PreTrainedModel):
    ...
    Params:
        config: a AudioConfig class instance with the configuration to build a new model
    ...
    Outputs: Tuple of (encoded_layers, pooled_output)
        `encoded_layers`: controled by `output_all_encoded_layers` argument:
            - `output_all_encoded_layers=True`: outputs a list of the full sequences of encoded-hidden-states at the end
                of each attention block (i.e. 12 full sequences for Audio-base, 24 for Audio-large), each
    ...
    config = modeling.AudioConfig(vocab_size_or_config_json_file=4096, hidden_size=768,
        num_hidden_layers=12, num_attention_heads=12, intermediate_size=3072)

    model = modeling.AudioModel(config=config)
