...
class TrainerBase(object):
    ...
    def load(self, path, loc=None):
        ...
        state_dict = torch.load(f"{path}.pth", map_location=loc)

        # Remap keys if necessary (your existing code)
        original_keys = list(state_dict.keys())
        for key in original_keys:
            if key.startswith("module.vis_encoder."):
                new_key = 'module.encoder.' + key[len("module.vis_encoder."):]
        ...
        model_state_dict = self.model.state_dict()  # Current model state dict
        filtered_state_dict = {}
        for key, value in state_dict.items():
            if key in model_state_dict:
                # Check if the shape matches
                if model_state_dict[key].shape == value.shape:
                    filtered_state_dict[key] = value  # Add if shapes match
                # else:
                #     if self.verbose:  # Optional: log mismatched keys
                #         print(f"Skipping key due to shape mismatch: {key} "
                #             f"(model: {model_state_dict[key].shape}, "
                #             f"checkpoint: {value.shape})")
            # else:
            #     if self.verbose:  # Optional: log missing keys
            #         print(f"Key not found in model: {key}")

        # Load filtered state_dict into the model
        results = self.model.load_state_dict(filtered_state_dict, strict=False)
