...
import clip
import math
from transformers import BartTokenizer, BartForConditionalGeneration, BartConfig, PegasusConfig, PegasusForConditionalGeneration, PegasusTokenizer
...
class MomentDataset(Dataset):
    def __init__(self, args, data_path, video_dir=None, video_feature_dir=None, asr_dir=None, asr_feature_dir=None, n_model_frames=-1, task=None):
        ...
        self.videoid2asr = {} ## dict with video id and corresponding asr content (start/end and content)
        if asr_dir is not None:
            ...
            for asr_path in self.asr_dir.glob('*.srt'):
                video_id = asr_path.stem                 ## Get the value of * (e.g., cvpr2016-paper1)
                with open(asr_path, 'r') as f:
                    transcript_srt_str = f.read()

                all_subs = []
                for sub in srt.parse(transcript_srt_str): ## Get the transcript value and make it to a tuple with start/end and content
                    all_subs.append(sub)
        ...
        ## Value of model frame: -1
        self.n_model_frames = n_model_frames

        self.tasks = ['moment_retrieval', 'memsum']
        self.task = task
        ...
        for i, (prompt, video_anns) in enumerate(prompt2video_anns.items()):
            ...
            for video_fname, video_ann in video_anns.items():
                ...
                datum['prompt'] = prompt
                datum['summary'] = video_ann['summary']

                # video_duration = video_ann['v_duration']
                
                ## Get the clip video feature paths
                video_feature_path = self.video_feature_dir / f'{video_fname}.pt' 
                video_duration = torch.load(video_feature_path, map_location='cpu').shape[0]
                datum['video_duration'] = video_duration 
                datum['n_model_frames'] = n_model_frames ## n_model_frames = -1
                datum['a_bounds'] = video_ann['bounds']

                if self.n_model_frames > 0: ## self.n_model_frames = -1
                ...
                #     step_start, step_end = video_ann['bounds']
                #     step_text = video_ann['summary'].strip() # step['heading'] is the summarization 
                ...
                #     task_datum["inputs"] = self.tokenizer(
                #         prompt,
                #         max_length=1024,
                #         truncation=True,
                #         padding='max_length',
                ...
                #         task_datum["label"] = self.tokenizer(
                #             step_text,
                #             max_length=150,
                #             truncation=True,
                #             padding='max_length',
                ...
                #     task_datum["inputs"] = {k: v.squeeze(0) for k, v in task_datum["inputs"].items()}
                #     task_datum["label"] = task_datum["label"]['input_ids'].squeeze(0)
                ...
                #     data.append(task_datum)
                    ...
                    # print("memsum data preparing")
                    moment_start = math.floor(video_ann['bounds'][0])
                    moment_end = math.floor(video_ann['bounds'][1])

                    #print(f"video duration: {video_duration}")
                    #print(f"n_frames: {n_frames}")

                    #print(f"moment in memsum: {moment_start, moment_end}")
                    target_text = video_ann['summary']

                    moment_mask = torch.zeros(n_frames, dtype=torch.long)
                    moment_mask[moment_start:moment_end] = 1
                    video_mask = torch.zeros(n_frames, dtype=torch.long)
                    video_mask[0:math.floor(video_duration)] = 1
                    
                    task_datum = deepcopy(datum)
                    task_datum['task'] = task
                    #task_datum: {'fname': 'ICCV2017-paper34.mp4', 
                    #'prompt': 'abc', 'video_duration': 211, 'n_model_frames': 700}
                    task_datum["target_text"] = target_text
                    task_datum['moment_mask'] = moment_mask
                    task_datum['video_mask'] = video_mask
                    #print(f"task_datum: {task_datum}")
                    data.append(task_datum)
    ...
    def __getitem__(self, index):
        ...
        # video_feat = self.load_feature(video_id, 'video')
        # # self.print_gpu_memory("After loading video features")
        # audio_feat = self.load_feature(video_id, 'audio')

        # # print(f"video and audio features before slicing: {video_feat.shape, audio_feat.shape}")

        # #切片
        # bounds = datum['a_bounds']
        # if video_feat is not None:
        #     # start_ratio = bounds[0] / video_duration
        #     # end_ratio = bounds[1] / video_duration

        #     # v_seq_len = video_feat.size(0)
        #     # v_start = int(start_ratio * v_seq_len)
        #     # v_end = int(end_ratio * v_seq_len)
        #     v_start, v_end = round(bounds[0]), round(bounds[1])
        #     video_feat = video_feat[v_start:v_end]

        # if audio_feat is not None:
        #     # start_ratio = bounds[0] / video_duration
        #     # end_ratio = bounds[1] / video_duration
            
        #     # a_seq_len = audio_feat.size(0)
        #     # a_start = int(start_ratio * a_seq_len)
        #     # a_end = int(end_ratio * a_seq_len)
        #     a_start, a_end = round(bounds[0]), round(bounds[1])
        #     audio_feat = audio_feat[a_start:a_end]
        
        # # print(f"video and audio features before trim: {video_feat.shape, audio_feat.shape}")

        
        # if video_feat is not None:
        #     video_feat = self.pad_or_trim_feature(video_feat, self.max_video_len)
        # if audio_feat is not None:
        #     audio_feat = self.pad_or_trim_feature(audio_feat, self.max_audio_len)

        # text = datum['prompt']
        # abstract = datum['summary']
        ...
        # out['video_features'] = video_feat
        # out['audio_features'] = audio_feat
        # out['video_name'] = video_id
    ...
    def collate_fn(self, batch):
        ...
        clip_text_ids = clip.tokenize(out_batch['prompts'], truncate=True)
    ...
    def clip4cap_get_text(self, caption=None):
        '''
        It tokenizes the main text (with [CLS] and [SEP] tokens) and ensures it is padded to a fixed length (max_words).
        If a caption is provided, it is tokenized, padded, and split into input and output sequences for a transformer-based model (likely for text generation or captioning).
        The function returns arrays containing the tokenized text, caption input/output IDs, and a decoder mask for further use in a model (such as a sequence-to-sequence model with attention).
        '''
        k = 1
...
def get_moment_loader(args, split='train', batch_size=32, task='moment_retrieval'):
    ...
    assert task in ['moment_retrieval', 'memsum'], task
    ...
    dataset = MomentDataset(
        ...
        data_path=data_path,
        video_dir=None,
        video_feature_dir=args.video_feature_dir,       # clip features
        asr_dir=args.asr_dir,                           # audio transcript text
        asr_feature_dir=args.asr_feature_dir,           # audio transcript features
        n_model_frames=args.n_model_frames,             # -1
        task=task,
    )
